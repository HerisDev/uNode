	...
	public static class ReflectionUtils {
		...
		public static bool CanCreateInstance(Type type) {
			...
			if(type.IsClass) {
				...
				if(ctor == null) {
					if(type.GetConstructors(publicAndNonPublicFlags).Any(constructor => constructor.GetParameters().All(p => p.IsOptional))) {
						return true;
					}
					return type.IsArray;
				}
		...
		public static ConstructorInfo GetDefaultConstructor(Type type) {
			...
			// Get the psudo default constructor if one exists, A constructor that has parameters but all parameters are optional.
			var psudoDefaultConstructor = type.GetConstructors(publicAndNonPublicFlags).FirstOrDefault(constructor => constructor.GetParameters().All(p => p.IsOptional));
			if(psudoDefaultConstructor != null) {
				return psudoDefaultConstructor;
			}
			else {
		...
		public static object CreateInstance(Type type, params object[] args) {
			...
			if(type.IsClass) {
				...
				if(ctor == null) {
					...
					// Handle the psudo default constructors.
					var psudoDefaultConstructor = type.GetConstructors(publicAndNonPublicFlags).FirstOrDefault(constructor => constructor.GetParameters().All(p => p.IsOptional));
					if(psudoDefaultConstructor != null)
						return psudoDefaultConstructor.Invoke(new object[psudoDefaultConstructor.GetParameters().Length]);
					return null;
				}
			}
					...
					if(preferedType.IsValueType) {
						...
						if(pType.Length == 0) {
							...
							for(int i = 0; i < pType.Length; i++) {
								...
								else if(preferedType.IsCastableTo(pType[i])) {
									return preferedType;
								}
							}
						}
